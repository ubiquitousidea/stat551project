...
\paragraph{}
Statisticians argue that the stock prices are normally distributed in the stock market. Therefore, in financial deratives pricing, Brownian motion with drift are often used. The parameters are the mean and the variance. In the bear market, the mean is higher and the variance is lower. Vice versa.\cite{lee}
...
hist(recession$diff, breaks = 1000, xlim = c(-500,500), 
     main = "Recession", 
     xlab = "Close price - Open price",col = "lightblue")
...
\paragraph{}
Knowing the current environment of the market gives the better prediction of the stock price. In this project, we would like to construct this market environment and better predict the stock price using this market environment.
\paragraph{}
We assume eanch of the stock has Gaussian distribution in a market environment and each market environment is modeled Hidden Markov Model(HMM) to obtain the discrete market regime. In this model, the state of the market regime changes according to a transition matrix.Therefore, we extract the market environment from the stock data.
